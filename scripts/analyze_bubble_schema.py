...
def collect_schema(obj: Any, path: str = "", schema: Dict[str, Set[type]] = None) -> Dict[str, Set[type]]:
    """
...
def collect_categorical_values(obj: Any, path: str = "", values: Dict[str, Set[Any]] = None) -> Dict[str, Set[Any]]:
    """
...
def identify_categorical_variables(values: Dict[str, Set[Any]], max_unique: int = 50) -> Dict[str, Set[Any]]:
    """
    ...
    for path, value_set in values.items():
        ...
        is_known_categorical = any(keyword in path.lower() for keyword in 
                                   ['type', 'status', 'tool', 'id', 'name', 'kind', 'category'])
...
def format_schema_report(schema: Dict[str, Set[type]], categorical: Dict[str, Set[Any]]) -> str:
    """
    ...
    for path in sorted_paths:
        types = schema[path]
        type_str = " | ".join(t.__name__ for t in sorted(types, key=lambda x: x.__name__))
        lines.append(f"- **`{path}`**: `{type_str}`")
    
    lines.append("")
    lines.append("## Categorical Variables and Values")
    lines.append("")
    lines.append("The following variables have limited unique values and are likely categorical:")
    lines.append("")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python analyze_bubble_schema.py <bubbles_json_file> [output_file]")
        sys.exit(1)
    
    input_file = Path(sys.argv[1])
    output_file = Path(sys.argv[2]) if len(sys.argv) > 2 else input_file.parent / f"{input_file.stem}_schema.md"
