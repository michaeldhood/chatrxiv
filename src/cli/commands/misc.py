...
def load_activity(ctx, file_path, db_path):
    """
    Load cursor activity data from a JSON or CSV file.
    
    The file should contain activity records with fields like:
    timestamp, activity_type, model, workspace_hash, composer_id,
    ...
    try:
        ...
        if file_path_obj.suffix.lower() == '.json':
            ...
            click.secho(f"Unsupported file format: {file_path_obj.suffix}. Use .json or .csv", fg='red', err=True)
            raise click.Abort()
...
@click.option(
    '--chart',
    type=click.Choice(['cost-over-time', 'cost-by-model', 'activity-timeline', 'cost-distribution', 'dashboard', 'all']),
    default='dashboard',
...
def estimate_costs(ctx, update_existing, db_path):
    ...
    try:
        ...
        if update_existing:
            ...
            cursor.execute("UPDATE chats SET estimated_cost = NULL")
            db.conn.commit()
        else:
            click.echo("Estimating costs for chats without existing estimates...")
        
        updated_count = estimator.update_chat_costs(db)
