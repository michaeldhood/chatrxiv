...
class ChatSearchService:
    ...
    def instant_search(
        self, query: str, limit: int = 10, sort_by: str = "relevance"
    ) -> List[Dict[str, Any]]:
        """
    ...
    def search_with_facets(
        ...
        tag_facets = self.db.get_search_tag_facets(
            query, tag_filters=None, workspace_filters=workspace_filters
        )

        # Get workspace facets (always show all available workspaces, even when filtering)
        workspace_facets = self.db.get_search_workspace_facets(
            query, tag_filters=tag_filters, workspace_filters=None
        )
